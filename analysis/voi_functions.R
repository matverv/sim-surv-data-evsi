...
surv_evsi_fun <- function (inb,
                           ...
                           seed,
                           sdists=NULL, 
                           wane_start_times=NULL, 
                           wane_end_times=NULL) {
  
  
  sapply(add_fu, function (t) { 
    
    start_t <- Sys.time() # start the clock
    ...
    if(is.null(pfs_curves)) {
      ...
      if(method == "standard_ana") {
        summ_stat <- weib_os_data_fun(theta_os, start_times_os, t, seed) 
      } 
      
      if(method == "standard_num") {
        summ_stat <- wane_weib_os_data_fun(theta_os, start_times_os, t, seed, sdists, wane_start_times, wane_end_times) 
      } 
    ...
    if(is.null(os_curves) == FALSE & is.null(pfs_curves) == FALSE) { #& is.null(start_times_os) == FALSE
      ...
      if(method == "standard_ana") {
        summ_stat <- weib_os_pfs_data_fun(theta_os, theta_pfs, start_times_os, start_times_pfs, t, seed) 
      } 

      if(method == "standard_num") {
        summ_stat <- wane_weib_os_pfs_data_fun(theta_os, theta_pfs, start_times_os, start_times_pfs, t, seed, sdists, wane_start_times, wane_end_times) 
      } 
    ...
    comp_time <- as.numeric(round(difftime(Sys.time(), start_t, units = "secs"), 0)) # stop the clock

    ######### Compute EVSI using GAM #########
    regr_model <- reg_mod_fun(summ_stat, arms, arm_indic)
    evsi <- evsi_fun(inb, summ_stat, regr_model)
    
    print(c("add. follow-up" = round(t,2), "evsi" = round(evsi[[1]],2), "se" = round(evsi[[2]],2), "lower" = round(evsi[[3]],2), "upper" = round(evsi[[4]],2), "Data-sim time (mins)" = comp_time )) 
    
    return(c("add. follow-up" = round(t,2), "evsi" = round(evsi[[1]],2), "se" = round(evsi[[2]],2), "lower" = round(evsi[[3]],2), "upper" = round(evsi[[4]],2), "Data-sim time (mins)" = comp_time)) 
